...
open class FilePickerPreference(context: Context, attrs: AttributeSet?) : Preference(context, attrs),
    ...
    private fun showFilePickerDialog() {
        ...
        if (mCurrentValue.isNotEmpty()) {
            ...
            if (f.exists()) {
                mCurrentFolder = f.parent ?: Environment.getExternalStorageDirectory().absolutePath
            } else {
    ...
    private fun populateList() {
        ...
        val txtFiles = File(mCurrentFolder).listFiles(FilenameFilter { _, name -> name.endsWith(".txt") })

...
class RobinsonProjection : PseudoCylindricalProjection() {
    /**
     ...
     * @return
     */
    private fun poly(a: DoubleArray, offset: Int, z: Double): Double {
    ...
    public override fun project(lplam: Double, lpphi: Double, xy: Point2D.Double): Point2D.Double {
        val phiAbs = abs(lpphi)
        // compute the spline segment index
        var i = floor(phiAbs * C1).toInt()
        if (i >= NODES) {
            i = NODES - 1
        }
        // evaluate spline segment at position dphi [0..RC1]
        val dphi = Math.toDegrees(phiAbs - RC1 * i)
        i *= 4
    ...
    public override fun projectInverse(x: Double, y: Double, lp: Point2D.Double): Point2D.Double {
        ...
        if (lp.y >= 1.0) {
            if (lp.y > 1.000001) {
                ...
                // Bernhard Jenny, May 26 2010
                lp.x = Double.Companion.NaN
                lp.y = Double.Companion.NaN
                return lp
                //throw new ProjectionException();
            } else {
                lp.y = if (y < 0.0) -MapMath.HALFPI else MapMath.HALFPI
                lp.x /= X!![4 * NODES]
            }
        } else {
            i = 4 * floor(lp.y * NODES).toInt()
            while (true) {
                if (Y!![i] > lp.y) {
                    i -= 4
                } else if (Y[i + 4] <= lp.y) {
            ...
            var Tc0: Double = Y[i]
            val Tc1: Double = Y[i + 1]
            val Tc2: Double = Y[i + 2]
            val Tc3: Double = Y[i + 3]
            ...
            // Bernhard Jenny, May 26 2010
            t = 5.0 * (lp.y - Tc0) / (Y[i + 4] - Tc0)
            Tc0 -= lp.y
            while (true) {
                // Newton-Raphson
                t1 =
                    (Tc0 + t * (Tc1 + t * (Tc2 + t * Tc3))) / (Tc1 + t * (Tc2 + Tc2 + t * 3.0 * Tc3))
                t -= t1
                if (abs(t1) < EPS) {
                    break
                }
    ...
    companion object {
        ...
        private val X: DoubleArray? = doubleArrayOf(
            1.0, -5.67239e-12, -7.15511e-05, 3.11028e-06,
        ...
        private val Y: DoubleArray? = doubleArrayOf(
            0.0, 0.0124, 3.72529e-10, 1.15484e-09,
         ...
         */
        private const val NODES = 18

        /**
         * global scale factor applied to the graticule
         */
        private const val FXC = 0.8487

        /**
         * height-to-width factor of the graticule
         */
        private const val HEIGHT_TO_WIDTH = 0.5072

        /**
         * vertical scale factor
         */
        private val FYC: Double = FXC * HEIGHT_TO_WIDTH * Math.PI // 1.3523;

        /**
         * convert from latitude in radians to spline segment index
         */
        private val C1: Double = NODES / Math.PI * 2 // 11.45915590261646417544;

        /**
         * a spline segment corresponds to this latitude segment in radians
         */
        private val RC1: Double = Math.PI / 2 / NODES // 0.08726646259971647884;

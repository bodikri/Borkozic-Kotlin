...
class McBrydeThomasFlatPolarQuarticProjection : PseudoCylindricalProjection() {
    ...
    override fun project(lplam: Double, lpphi: Double, out: Point2D.Double): Point2D.Double {
        ...
        for (i in NITER downTo 1) {
            val th1 = (Math.sin(0.5 * lpphi) + Math.sin(lpphi) - c) /
                    (0.5 * Math.cos(0.5 * lpphi) + Math.cos(lpphi))
            out.y -= th1
            if (Math.abs(th1) < EPS) {
                break
            }
        }
        out.x = FXC * lplam * (1.0 + 2.0 * Math.cos(lpphi) / Math.cos(0.5 * lpphi))
